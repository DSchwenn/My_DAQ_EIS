...
y1_vals = np.array([0, 1, 3, 2, 3, 5])
y2_vals = np.array([0, 1, 3, 2, 3, 5])
...
def animate(i):
    ...
    r2 = np.random.rand(1,5)
    xx = x_vals[-1]+np.array([1, 2, 3, 4, 5])

    x_vals = np.append(x_vals, xx)#np.array(next(index))+5 )
    y1_vals = np.append(y1_vals,r1 )
    y2_vals = np.append(y2_vals, r2 )
...
ani = FuncAnimation(plt.gcf(), animate, interval=50) # run animate function every 1000 ms
